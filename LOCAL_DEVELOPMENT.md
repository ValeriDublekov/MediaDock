...
### Proposal Application Checks

`--mode all` generates audit proposals but never applies them. Proposal
application accepts exactly one explicit proposal ID; no local script supports
bulk application. Exercise the plan against fake repositories or the Firestore
emulator first:

```bash
python -m movies_feed.cli --config legacy/config.json --fake-repos --mode apply-proposals --proposal-id <proposal-id> --dry-run
```
...
larger groups at that bound. Stale evidence must be reviewed and regenerated by
rerunning `recheck-existing`; a failed proposal is terminal and also requires a
newly generated, reviewed, and approved proposal ID. Firestore commits each
proposal atomically, so a failed transaction rolls back all catalog changes and
does not require manual reconstruction.
